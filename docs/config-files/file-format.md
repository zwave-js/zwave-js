...
| `recommendedValue` | number  |    no     | An optional recommended value for this parameter, which may provide better device performance or behavior than the default value and benefit most users. This value is only automatically applied during device interview when the `applyConfigurationRecommendedValues` driver option is enabled. |

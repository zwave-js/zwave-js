...
| `hidden`           | boolean |    no     | Whether this parameter should be completely filtered out during configuration loading. Hidden parameters will not have metadata created or values persisted. Use this for factory debug parameters, calibration data, or other settings that should not be exposed to end users. See [Hidden Parameters](config-files/hidden-parameters.md) for details. |

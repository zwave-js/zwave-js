...
${manufacturers
	.map((m) => {
		...
		if (m.formerNames) {
			...
			former = ` // formerly: ${m.formerNames
				.map((f) => f.formerName)
				.join(", ")}`;
		}
		return `	"${formatId(m.id)}": "${m.name.replaceAll(
			'"',
			'\\"',
		)}",${former}`;
	})

...
// Keep model, revision and dtype in sync with the zwave-js-ui repository
// (.github/bot-scripts/localEmbeddings.cjs there) and the dev MCP server's
// semantic search (packages/mcp-server-dev/src/semantic/env.ts), so evals
// and answer quality stay comparable
...
function defaultModelCacheDir() {
	...
	const base = xdg && xdg.trim().length > 0
		? xdg
		: join(homedir(), ".cache");
	return join(base, "zwave-js-bot", "models");
}

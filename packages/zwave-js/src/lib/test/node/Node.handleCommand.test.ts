...
test.sequential("should map commands from the root endpoint to endpoint 1 if configured", async (t) => {
	const node = makeNode([
		[
...
test.sequential("a notification event is sent when receiving an EntryControlNotification", async (t) => {
	const node = makeNode([
		[CommandClasses["Entry Control"], { isSupported: true, version: 1 }],
	]);
	...
	const command = (await CommandClass.parse(buf, {

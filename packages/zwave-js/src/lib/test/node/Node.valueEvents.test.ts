...
test.sequential("the emitted events should contain a speaking name for the CC", ({
	context,
	expect,
}) => {
	const { node } = context;
...
test.sequential("the emitted events should contain a speaking name for the propertyKey", ({
	context,
	expect,
}) => {
	const { node } = context;
...
test.sequential("the emitted events should not be emitted for internal values", ({
	context,
	expect,
}) => {
	const { node } = context;

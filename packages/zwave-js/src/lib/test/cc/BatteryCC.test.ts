...
integrationTest(
	"When the battery is disconnected, level and chargingStatus values are removed but metadata is preserved",
	{
		...
		async testBody(t, driver, node, mockController, mockNode) {
			...
			t.expect(node.getValue(BatteryCCValues.level.id)).toBeUndefined();
			t.expect(
				node.getValue(BatteryCCValues.chargingStatus.id),
			).toBeUndefined();

			// But metadata should still be present
			t.expect(
				node.getValueMetadata(BatteryCCValues.level.id),
			).toMatchObject({ label: "Battery level" });
			t.expect(
				node.getValueMetadata(BatteryCCValues.chargingStatus.id),
			).toMatchObject({ label: "Charging status" });

			// Other values should still be persisted normally
			t.expect(node.getValue(BatteryCCValues.rechargeable.id)).toBe(true);

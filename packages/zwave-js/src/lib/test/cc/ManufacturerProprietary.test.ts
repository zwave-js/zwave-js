...
integrationTest("sendAndReceiveData works", {
	// debug: true,

	nodeCapabilities: {
		commandClasses: [CommandClasses["Manufacturer Proprietary"]],
	},
		...
		const respondToManufacturerProprietary: MockNodeBehavior = {
			handleCC(controller, self, receivedCC) {
				if (
					...
					const received = receivedCC as ManufacturerProprietaryCC;
	...
	async testBody(t, driver, node, mockController, mockNode) {
		...
		const response = await node.commandClasses[
			"Manufacturer Proprietary"
		].sendAndReceiveData(manufacturerId, data);

...
integrationTest("successful supervised setValue(255): expect verify GET", {
	nodeCapabilities: {
		commandClasses: [
	...
	customSetup: async (driver, controller, mockNode) => {
		...
		const respondToSupervisionGetWithDuration: MockNodeBehavior = {
			handleCC(controller, self, receivedCC) {
				if (
					...
					receivedCC.encapsulated instanceof MultilevelSwitchCCSet &&
		...
		{
			let call = 0;
			const responses: [target: number, current: MaybeUnknown<number>][] =
				[
					// during interview
					[20, 20],

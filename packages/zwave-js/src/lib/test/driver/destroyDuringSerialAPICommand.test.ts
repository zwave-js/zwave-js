...
test.sequential("destroying the driver while a Serial API command waits for its ACK aborts the command", async (t) => {
...
test.sequential("destroying the driver while a Serial API command is being written does not cause an unhandled rejection", async (t) => {

...
integrationTestMulti("Inclusion with S2 should work", {
	// debug: true,
	...
	testBody: async (t, driver, nodes, mockController, mockNodes) => {
		...
		t.expect(includedNode!.supportsCC(CommandClasses["Security 2"])).toBe(
			true,
		);

		// That Binary Switch CC is only supported securely
		t.expect(
			includedNode?.isCCSecure(CommandClasses["Binary Switch"]),
		).toBe(true);
...
integrationTestMulti("Inclusion with S0 should work", {
	// debug: true,
	...
	testBody: async (t, driver, nodes, mockController, mockNodes) => {
		...
		t.expect(includedNode!.id).toBe(3);
		t.expect(includedNode!.isSecure).toBe(true);
		t.expect(includedNode!.supportsCC(CommandClasses.Security)).toBe(true);

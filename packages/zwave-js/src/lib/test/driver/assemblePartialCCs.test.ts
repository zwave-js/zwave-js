...
test("returns the message when a non-partial CC is received", async ({
	context,
	expect,
}) => {
...
test("returns the message when a partial CC is received that expects no more reports", async ({
	context,
	expect,
}) => {
	const { driver } = context;
...
test("returns undefined when a partial CC is received that expects more reports", async ({
	context,
	expect,
}) => {
	const { driver } = context;
...
test("returns the final message when the final partial CC is received and merges its data", async ({
	context,
	expect,
}) => {
	const { driver } = context;
	...
	await expect(driver["assemblePartialCCs"](msg1)).resolves.toBeUndefined();
	...
	expect((msg2.command as AssociationCCReport).nodeIds).toStrictEqual([
		1, 2, 3, 4, 5, 6,
	]);
});

test("does not crash when receiving a Multi Command CC", async ({
	context,
	expect,
}) => {
	const { driver } = context;
...
test("supports nested partial/partial CCs (part 1)", async ({
	context,
	expect,
}) => {
	const { driver } = context;
...
test("supports nested partial/partial CCs (part 2)", async ({
	context,
	expect,
}) => {
	const { driver } = context;
...
test("returns undefined when a partial CC throws Deserialization_NotImplemented during merging", async ({
	context,
	expect,
}) => {
	const { driver } = context;
...
test("returns undefined when a partial CC throws CC_NotImplemented during merging", async ({
	context,
	expect,
}) => {
	const { driver } = context;
...
test("returns undefined when a partial CC throws PacketFormat_InvalidPayload during merging", async ({
	context,
	expect,
}) => {
	const { driver } = context;
...
test("passes other errors during merging through", async ({
	context,
	expect,
}) => {
	const { driver } = context;
	...
	await expect(() => driver["assemblePartialCCs"](msg)).rejects.toThrow(
		"invalid",
	);
});

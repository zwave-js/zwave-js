...
integrationTest("Device Specific Get returns a configured string device ID", {
	...
	testBody: async (t, driver, node, mockController, mockNode) => {
		for (const deviceIdType of [
			...
			t.expect(response.deviceId).toBe(vendorWithStringDeviceId.deviceId);
		}
	},
});

integrationTest("Device Specific Get returns a configured binary device ID", {
	...
	testBody: async (t, driver, node, mockController, mockNode) => {
		...
		await mockNode.sendToController(createMockZWaveRequestFrame(request));

...
integrationTest("S0 Commands Supported Get is answered on an S2-enabled node", {
	nodeCapabilities: {
		commandClasses: [CommandClasses.Security, CommandClasses["Security 2"]],
	...
	testBody: async (_t, _driver, node, mockController, mockNode) => {
		...
		mockNode.assertReceivedControllerFrame(
			...
			{
				errorMessage: "Expected an empty S0 Commands Supported Report",
			},
		);
	},
});
...
integrationTest(
	"S0 Commands Supported Report is accepted from an S2-enabled node",
	{
		...
		testBody: async (t, _driver, node) => {
			...
			await t
				.expect(node.commandClasses.Security.getSupportedCommands())

...
const test = baseTest.extend<LocalTestContext>({
	context: [
		async ({}, use) => {
			...
			// Create a mock driver with security manager
...
test("secureBootstrapS0 should add Security CC when assumeSupported is true", async ({ context, expect }) => {
	...
	// This is the behavior that was missing in the replaceFailedNodeTask method
	// as described in:
	...
	// Call the secureBootstrapS0 method with assumeSupported=true

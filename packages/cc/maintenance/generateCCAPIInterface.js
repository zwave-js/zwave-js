...
export async function generateCCAPIInterface() {
    const ccFiles = (await fs.readdir(ccDir)).filter((file) => file.endsWith(".ts")
    ...
    let endTokenStart = apiFileContent.indexOf(endTokenInterface, startTokenEnd);
    apiFileContent = apiFileContent.slice(0, startTokenEnd)
        + "\n"
        + CCsWithAPI.map(({ name, className, file }) => `\t${name}: import("${path
            .relative(libDir, ccDir)
            .replaceAll("\\", "/")}/${file}").${className};`).join("\n")
        + "\n"
...
if (esMain(import.meta))
    void generateCCAPIInterface();
//# sourceMappingURL=generateCCAPIInterface.js.map
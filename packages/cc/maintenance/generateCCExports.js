 ...
 */
import { formatWithDprint, hasComment, loadTSConfig, projectRoot, } from "@zwave-js/maintenance";
...
function hasPublicAPIComment(node, sourceFile) {
...
function findExports() {
    ...
    const ccExports = new Map();
    function addExport(filename, name, typeOnly) {
        if (!ccExports.has(filename))
            ccExports.set(filename, []);
        ccExports.get(filename).push({ name, typeOnly });
    }
    function inheritsFromCommandClass(node) {
        let type = checker.getTypeAtLocation(node);
        while (type) {
            if (type.symbol.name === "CommandClass")
                return true;
            type = checker.getBaseTypes(type)[0];
        }
        return false;
    }
    ...
    for (const sourceFile of program.getSourceFiles()) {
        ...
        ts.forEachChild(sourceFile, (node) => {
            ...
            if (ts.isEnumDeclaration(node)
                ...
                if (!hasPublicAPIComment(node, sourceFile))
                    return;
                // Make sure we're trying to access a node that is actually exported
                if (!node.modifiers?.some((m) => m.kind === ts.SyntaxKind.ExportKeyword)) {
                    const location = ts.getLineAndCharacterOfPosition(sourceFile, node.getStart(sourceFile, false));
                    throw new Error(`${relativePath}:${location.line} Found @publicAPI comment, but the node ${node.name.text} is not exported!`);
                }
                addExport(sourceFile.fileName, node.name.text, ts.isTypeAliasDeclaration(node)
                    || ts.isInterfaceDeclaration(node));
            }
                ...
                for (const exportSpecifier of node.exportClause.elements) {
                    addExport(sourceFile.fileName, exportSpecifier.name.text, node.isTypeOnly || exportSpecifier.isTypeOnly);
                }
            }
                ...
                && (hasPublicAPIComment(node, sourceFile)
                    ...
                    || node.declarationList.declarations.some((d) => d.name.getText().endsWith("CCValues")))) {
                for (const variable of node.declarationList.declarations) {
                    if (ts.isIdentifier(variable.name)) {
                        addExport(sourceFile.fileName, variable.name.text, false);
                    }
                }
            }
        });
...
export async function generateCCExports() {
    let fileContent = `
...
`;
    ...
    for (const [filename, fileExports] of [...findExports().entries()].sort(([fileA], [fileB]) => compareStrings(fileA, fileB))) {
        const relativePath = path
        ...
        if (valueExports.length) {
            ...
            fileContent += `export { ${valueExports
                .map((e) => e.name)
                .join(", ")} };\n`;
            registerFunctionContent += `void ${valueExports
                .map((e) => e.name)
                .join(", ")};\n`;
        }
    }
...
`;
    ...
    const originalFileContent = await fs.readFile(ccIndexFile, "utf8").catch(() => undefined);
...
if (esMain(import.meta))
    void generateCCExports();
//# sourceMappingURL=generateCCExports.js.map
...
export async function generateCCAPIInterfaceFile(
	...
	let content = `// This file is auto-generated by maintenance/generateCCAPIInterface.ts
...
`;
	...
	for (const { name, className, file } of CCsWithAPI) {
		const quotedName = name.startsWith(`"`) ? name : `"${name}"`;
		content += `\t${quotedName}: import("${ccRelativePath}/${file}").${className};\n`;
	}
...
`;
	...
	return new Map([
		[
			...
			await formatWithOxfmt(generatedFile, content),
		],
	]);
}

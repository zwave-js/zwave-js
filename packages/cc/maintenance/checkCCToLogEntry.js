...
export function checkCCToLogEntry() {
    ...
    for (const sourceFile of program.getSourceFiles()) {
        ...
        ts.forEachChild(sourceFile, (node) => {
            ...
            if (ts.isClassDeclaration(node)
                ...
                else if (node.getText(sourceFile).includes("// @noLogEntry")) {
                    // Check disabled with a `// @noLogEntry` comment in the class body
                    results.set(node.name.text, "ignored");
                }
                else {
                    const hasToLogEntry = node.members.some((member) => ts.isMethodDeclaration(member)
    ...
    for (const cc of sortedCCs) {
        ...
        console.error(`- [${checkResult !== false ? "x" : " "}] ${cc}${checkResult === "empty"
            ? " _(empty CC)_"
...
//# sourceMappingURL=checkCCToLogEntry.js.map
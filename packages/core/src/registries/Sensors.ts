...
export function getSensor<Key extends number>(
	type: Key,
): Key extends keyof Sensors ? Sensors[Key] : Sensor | undefined {
...
export function getSensorName(sensorType: number): string {
	return getSensor(sensorType)?.label ?? `UNKNOWN (${num2hex(sensorType)})`;
}
...
>(
	...
	const scaleDef: ScaleDefinition | undefined = (sensor?.scales as any)?.[
		scale
	];
...
export function getAllSensorScales(
	...
	return Object.entries(sensor.scales).map(([key, scale]) => ({
		key: parseInt(key, 10),
		...scale,
	}));
}

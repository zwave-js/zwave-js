...
const deviceClasses: Record<number, GenericDeviceClassDefinition> =
	Object.freeze({
		[0x01]: {
...
export function getSpecificDeviceClass(
	...
	return {
		...
		zwavePlusDeviceType:
			specificClass.zwavePlusDeviceType
			?? genericClass.zwavePlusDeviceType,
		requiresSecurity:
			specificClass.requiresSecurity
			?? genericClass.requiresSecurity
			?? false,
		maySupportBasicCC:
			specificClass.maySupportBasicCC
			?? genericClass.maySupportBasicCC
			?? true,
		isSlowActuator:
			specificClass.isSlowActuator

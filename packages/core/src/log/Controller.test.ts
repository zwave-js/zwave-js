...
test.sequential("ControllerLogger.value() -> prints a short tag for the change type", ({
	context,
	expect,
}) => {
...
test.sequential("ControllerLogger.value() -> prints a tag including the CC name", ({
	context,
	expect,
}) => {
	const { controllerLogger, spyTransport } = context;
...
test.sequential("ControllerLogger.value() -> prints a tag including the Node ID", ({
	context,
	expect,
}) => {
	const { controllerLogger, spyTransport } = context;
...
test.sequential("ControllerLogger.value() -> prints a secondary tag including the CC endpoint", ({
	context,
	expect,
}) => {
	const { controllerLogger, spyTransport } = context;
...
test.sequential("ControllerLogger.value() -> prints a secondary tag if the value is internal", ({
	context,
	expect,
}) => {
	const { controllerLogger, spyTransport } = context;
...
test.sequential("ControllerLogger.value() -> prints the name of the property", ({
	context,
	expect,
}) => {
	const { controllerLogger, spyTransport } = context;
...
test.sequential("ControllerLogger.value() -> prints the name and key of map-like properties", ({
	context,
	expect,
}) => {
	const { controllerLogger, spyTransport } = context;
...
test.sequential("ControllerLogger.value() -> prints a the value change according to the change type", ({
	context,
	expect,
}) => {
	const { controllerLogger, spyTransport } = context;
...
test.sequential("ControllerLogger.value() -> stringifies objects", ({
	context,
	expect,
}) => {
	const { controllerLogger, spyTransport } = context;
...
test.sequential("ControllerLogger.metadata() -> prints a tag including the CC name", ({
	context,
	expect,
}) => {
	const { controllerLogger, spyTransport } = context;
...
test.sequential("ControllerLogger.metadata() -> prints a tag including the Node ID", ({
	context,
	expect,
}) => {
	const { controllerLogger, spyTransport } = context;
...
test.sequential("ControllerLogger.metadata() -> prints a secondary tag including the CC endpoint", ({
	context,
	expect,
}) => {
	const { controllerLogger, spyTransport } = context;
...
test.sequential("ControllerLogger.metadata() -> prints a secondary tag if the value is internal", ({
	context,
	expect,
}) => {
	const { controllerLogger, spyTransport } = context;
...
test.sequential("ControllerLogger.metadata() -> it prints the name of the property", ({
	context,
	expect,
}) => {
	const { controllerLogger, spyTransport } = context;
...
test.sequential("ControllerLogger.metadata() -> prints the change type", ({
	context,
	expect,
}) => {
	const { controllerLogger, spyTransport } = context;
...
test.sequential("ControllerLogger.interviewStage() -> includes a tag for the node ID", ({
	context,
	expect,
}) => {
	const { controllerLogger, spyTransport } = context;
...
test.sequential("ControllerLogger.interviewStage() -> logs the name of the interview stage", ({
	context,
	expect,
}) => {
	const { controllerLogger, spyTransport } = context;
...
test.sequential("ControllerLogger.interviewStage() -> prints a custom message when the interview is complete", ({
	context,
	expect,
}) => {
	const { controllerLogger, spyTransport } = context;
...
test.sequential("ControllerLogger.interviewStart() -> includes a tag for the node ID", ({
	context,
	expect,
}) => {
	const { controllerLogger, spyTransport } = context;
...
test.sequential("ControllerLogger.interviewStart() -> logs the name of the last interview stage", ({
	context,
	expect,
}) => {
	const { controllerLogger, spyTransport } = context;
...
test.sequential("ControllerLogger.logNode() -> logs short messages correctly", ({
	context,
	expect,
}) => {
	const { controllerLogger, spyTransport } = context;
...
test.sequential("ControllerLogger.logNode() -> logs long messages correctly", ({
	context,
	expect,
}) => {
	const { controllerLogger, spyTransport } = context;
...
test.sequential("ControllerLogger.logNode() -> logs with the given loglevel", ({
	context,
	expect,
}) => {
	const { controllerLogger, spyTransport } = context;
...
test.sequential("ControllerLogger.logNode() -> has a default loglevel of info", ({
	context,
	expect,
}) => {
	const { controllerLogger, spyTransport } = context;
...
test.sequential("ControllerLogger.logNode() -> logs the direction prefix", ({
	context,
	expect,
}) => {
	const { controllerLogger, spyTransport } = context;
...
test.sequential("ControllerLogger.print() -> logs short messages correctly", ({
	context,
	expect,
}) => {
	const { controllerLogger, spyTransport } = context;
...
test.sequential("ControllerLogger.print() -> logs long messages correctly", ({
	context,
	expect,
}) => {
	const { controllerLogger, spyTransport } = context;
...
test.sequential("ControllerLogger.print() -> logs with the given loglevel", ({
	context,
	expect,
}) => {
	const { controllerLogger, spyTransport } = context;
...
test.sequential("ControllerLogger.print() -> has a default loglevel of info", ({
	context,
	expect,
}) => {
	const { controllerLogger, spyTransport } = context;

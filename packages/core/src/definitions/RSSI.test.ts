...
test("averageRSSI should calculate exponential moving average correctly", () => {
	...
	// Expected: -51 * 0.9 + (-40) * 0.1 = -45.9 + (-4) = -49.9 → -50 (rounded)
...
test("averageRSSI should handle RSSI errors correctly", () => {
	...
	expect(averageRSSI(-50, RssiError.ReceiverSaturated, 0.9)).toBe(Math.round(-50 * 0.9 + 0 * 0.1));

	// Test with NoSignalDetected error - should treat as -128 dBm
	expect(averageRSSI(-50, RssiError.NoSignalDetected, 0.9)).toBe(Math.round(-50 * 0.9 + (-128) * 0.1));
});

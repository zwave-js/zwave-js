...
export class RCPSerialStreamFactory {
	...
	public async createStream(): Promise<RCPSerialStream> {
		// Set up streams for the underlying resource
		const { source, sink } = await this.binding();
		return new RCPSerialStream(source, sink, this.logger);
	}
}
	...
	>
{
	...
	public readonly readable: ReadableStream<RCPSerialFrame>;

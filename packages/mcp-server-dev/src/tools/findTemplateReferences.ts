...
async function handleFindTemplateReferences(
	...
	try {
		const files = (await listConfigFiles()).map((f) =>
			join(DEVICES_DIR, f),
		);
		...
		return textResult(
			`${refs.length} reference${
				...
				+ refs.map((r) => `${r.file}:${r.line}:${r.column}`).join("\n"),
		);
	} catch (error: any) {
...
export const findTemplateReferencesTool: ToolHandler<FindTemplateReferencesArgs> =
	{
		name: TOOL_NAME,
		description:

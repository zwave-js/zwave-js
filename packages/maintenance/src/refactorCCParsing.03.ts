...
async function main() {
	...
	for (const file of sourceFiles) {
		...
		const hasWithAddressImport = file
			.getImportDeclarations()
			.some((decl) =>
				decl
					.getNamedImports()
					.some((imp) => imp.getName() === "WithAddress"),
			);
		...
		const oldImports = file
			...
			.map((decl) =>
				decl
					.getNamedImports()
					.find((imp) => imp.getName() === "CCCommandOptions"),
			)
		...
		const oldTypes = file
			...
			.filter(
				(typ): typ is IntersectionTypeNode =>
					typ != undefined &&
					!!typ.getParent().isKind(SyntaxKind.Parameter) &&
					!!typ
						.getParent()
						.getParent()
						?.isKind(SyntaxKind.Constructor),
			);

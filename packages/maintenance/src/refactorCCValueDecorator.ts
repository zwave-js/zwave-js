...
async function main() {
	...
	for (const file of sourceFiles) {
		const ccValueImport = file
			.getImportDeclarations()
			.find((i) =>
				i
					.getModuleSpecifierValue()
					.endsWith("CommandClassDecorators.js"),
			)
			?.getNamedImports()
			.find((n) => n.getName() === "ccValue");
		if (!ccValueImport) continue;

		ccValueImport.replaceWithText("ccValueProperty");

		const classesWithDecoratedProperties = file
			.getDescendantsOfKind(SyntaxKind.ClassDeclaration)
			.map((c) => {
				const decoratedProperties = c
					.getDescendantsOfKind(SyntaxKind.PropertyDeclaration)
					.map((p) => {
						const decorator = p.getDecorator("ccValue");
						if (!decorator) return;

						return [p, decorator] as const;
					})
			...
			for (const [prop, dec] of propAndDec) {
				if (dec.getArguments().length === 2) {
					...
					if (bodyText.includes("as const")) {
						secondArg
							.getBody()
							.replaceWithText(bodyText.replace("as const", ""));
					}
				}
				...
				const newDecorator: OptionalKind<DecoratorStructure> = {
					...
					arguments: [`"${prop.getName()}"`, ...decArgs],
				};

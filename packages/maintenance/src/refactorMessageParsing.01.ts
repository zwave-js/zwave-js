...
async function main() {
	...
	for (const file of sourceFiles) {
				...
				(iface) =>
					[
						...
						iface
							.getExtends()
							.filter(
								(ext) => ext.getText() === "MessageBaseOptions",
							),
					] as const,
			)
		...
		const ctors = msgImplementations
			.map((cls) => {
				...
				if (cls.getName()!.startsWith("SendDataMulticastRequest"))
					debugger;

				// with a union type where one is MessageDeserializationOptions
				const types = ctorParam
		...
		const hasRawImport = file
			...
			.some((decl) =>
				decl
					.getNamedImports()
					.some((imp) => imp.getName() === "MessageRaw"),
			);
		if (!hasRawImport) {
		...
		const hasContextImport = file
			.getImportDeclarations()
			.some((decl) =>
				decl
					.getNamedImports()
					.some((imp) => imp.getName() === "MessageParsingContext"),
			);
		if (!hasContextImport) {
			const existing = file.getImportDeclaration((decl) =>
				decl.getModuleSpecifierValue().startsWith("@zwave-js/serial"),
		...
		for (const [clsName, ctor, otherType, ifStatement] of ctors) {
			...
			if (otherType === "MessageBaseOptions") {
				ctor.getParameters()[0].setType("MessageBaseOptions");
			} else if (otherType) {
			...
			const parseImplBlock = ifStatement
				? ifStatement.getThenStatement().asKind(SyntaxKind.Block)!
				: methodBody;
			...
			for (const [expr, left, identifier, value] of assignments) {
				if (!properties.has(identifier)) {
					...
					let valueType: string | undefined = value
						.getType()
						.getText()
						.replaceAll(/import\(.*?\)\./g, "");
					const prop = ctor.getParent().getProperty(identifier);
					if (prop) {
						valueType = prop
							.getType()
							.getText()
			...
			const thisAccesses = parseImplBlock
				.getDescendantsOfKind(SyntaxKind.PropertyAccessExpression)
				.filter(
					(expr) =>
			...
			const optionsDotCtx = parseImplBlock
				.getDescendantsOfKind(SyntaxKind.PropertyAccessExpression)

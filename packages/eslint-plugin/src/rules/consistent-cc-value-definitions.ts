...
(arg as TSESTree.TSAsExpression)
.expression,
),
);
},
...
body,
context.sourceCode.getText(
(body as TSESTree.TSAsExpression)
.expression,
),
);
},
...
messages: {
"no-as-const":
"Unnecessary `as const` in CC value definition. The V helper type parameters are already `const`-annotated.",
},
},
defaultOptions: [],
},

...
test.sequential("syncExternalConfigDir() syncs the external config dir if it does not exist", async ({
	context,
	expect,
}) => {
	...
	expect(await fsp.readFile(path.join(configDir, "version"), "utf8")).toBe(
		ownVersion,
	);
}, 60000);

test.sequential("syncExternalConfigDir() syncs the external config dir alone if it is from an incompatible version", async ({
	context,
	expect,
}) => {
	const { tempDir, logger } = context;
	...
	await fsp.writeFile(path.join(configDir, "version"), otherVersion!, "utf8");
...
test.sequential("syncExternalConfigDir() leaves the external config dir alone if it is from a newer compatible version", async ({
	context,
	expect,
}) => {
	const { tempDir, logger } = context;
...
test.sequential("loading config files from the embedded config dir", async ({
	context,
	expect,
}) => {
...
test.sequential("loading config files from the ZWAVEJS_EXTERNAL_CONFIG", async ({
	context,
	expect,
}) => {
...
test.sequential("loading config from the deviceConfigPriorityDir (without ZWAVEJS_EXTERNAL_CONFIG)", async ({
	context,
	expect,
}) => {
...
test.sequential("loading config from the deviceConfigPriorityDir (with ZWAVEJS_EXTERNAL_CONFIG)", async ({
	context,
	expect,
}) => {
...
test.sequential(`config files with the "preferred" flag are preferred`, async ({
	context,
	expect,
}) => {
	const { logContainer } = context;
